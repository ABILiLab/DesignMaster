...
from model_mc_v3.dynamics_mc_v3 import DynamicsEfficient
from model_base.noise import GammaNetwork, PredefinedNoiseSchedule
...
class EDM_CFG(torch.nn.Module):
    def __init__(
            ...
            condition_drop_prob=0.1  # Probability of dropping conditions for training
    ):
        super().__init__()
        ...
        self.dynamics = DynamicsEfficient(
            device=device,
            ...
            condition_drop=condition_drop_prob
        )
        self.device = device
    ...
    def forward(self, data):
        ...
        # Prepare numerical properties for conditioning
        ...
        properties = [molecular_weight, h_bond_acceptor, h_bond_donor, log_p, rotatable_bond_count,
        ...
        # During training, randomly apply either conditional or unconditional denoising
        if self.training:
            ...
            eps_t_hat = self.dynamics.forward(
                ...
                categorical_context=[context],
                numerical_context=properties,
                edge_mask=edge_mask,
            )
        else:
            # During inference, use classifier-free guidance
            eps_t_hat = self.dynamics.classifier_free_guidance_forward(
                xh=z_t,
                t=t,
                node_mask=node_mask,
                linker_mask=linker_mask,
    ...
    @torch.no_grad()
    def sample_chain(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context,
                     numerical_context, keep_frames=None, guidance_scale=None):
        """
        ...
        original_guidance_scale = self.dynamics.guidance_scale
        # Set new value for this sampling
        self.dynamics.guidance_scale = guidance_scale
        ...
        # Restore original guidance scale
        self.dynamics.guidance_scale = original_guidance_scale

        return chain

    def sample_p_zs_given_zt_only_linker(self, s, t, z_t, node_mask, fragment_mask, linker_mask, edge_mask, context,
                                         numerical_context, guidance_scale=None):
        ...
        # Neural net prediction with classifier-free guidance
        if guidance_scale is not None:
            # Store original value
            original_guidance_scale = self.dynamics.guidance_scale
            # Set new value for this sampling step
            self.dynamics.guidance_scale = guidance_scale

            eps_hat = self.dynamics.classifier_free_guidance_forward(
                xh=z_t,
                t=t,
                node_mask=node_mask,
                linker_mask=linker_mask,
            ...
            # Use regular forward pass with conditions
    ...
    # Rest of the methods remain the same as in the original EDM class
    ...
    def gaussian_kl(q_mu, q_sigma, p_mu, p_sigma):
        ...
        return EDM_CFG.sum_except_batch(kl)
    ...
    def gaussian_kl_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
        ...
        Returns:
            The KL distance, summed over all dimensions except the batch dim.
        """
        mu_norm_2 = EDM_CFG.sum_except_batch((q_mu - p_mu) ** 2)
