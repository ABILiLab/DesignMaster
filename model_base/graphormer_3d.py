...
class GraphormerBlock(nn.Module):
    ...
    def forward(
        ...
        graph_attn_bias = (self.edge_fc1(edge_attr) * edge_mask).view(bs, n_nodes, n_nodes,
                                                                      self.attention_heads).permute(0, 3, 1, 2).reshape(
            bs * self.attention_heads, n_nodes, n_nodes)
        graph_attn_weight = (self.edge_fc2(edge_attr) * edge_mask).view(bs, n_nodes, n_nodes,
                                                                        self.attention_heads).permute(0, 3, 1,
                                                                                                      2).reshape(
            bs * self.attention_heads, n_nodes, n_nodes)
